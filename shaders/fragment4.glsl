...
vec4 premium_star() {
  ...
  vec3 vTextureNormal = normalize(texture(u_NormalMap, (vUV + vec2(-f_xOffset, f_xOffset)) * 2.0).xyz * 2.0 - 1.0);
  ...
  vec4 backgroundColor = texture(u_BackgroundTexture, vec2(gradientPosition.x + (gl_FragCoord.x / resolution.x) * gradientPosition.y, gradientPosition.z + (1.0 - (gl_FragCoord.y / resolution.y)) * gradientPosition.w));
...
vec4 golden_star() {
  ...
  vec3 flecksNormal = normalize(1.0 - texture(u_NormalMap, (vUV + .7 * vec2(-f_xOffset, f_xOffset)) * 2.0).xyz);
...
void main() {
    if (golden <= 0.0)
        fragColor = premium_star();
    else if (golden >= 0.0)
        fragColor = golden_star();
    else
        fragColor = mix(premium_star(), golden_star(), golden);
}